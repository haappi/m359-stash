        ...
        for (Button[] outer : buttons
            ...
            for (Button button : outer) {
                button.setOnMouseClicked((event -> {
                    ...
                    BasicBuilding building = new BasicBuilding(xClicked, yClicked, 5, Color.YELLOW, buttons, gridPane, selectedBuilding);
                        ...
                        for (int j = lion.getY() - 2; j <= lion.getY() + 2; j++) {
                            if (i >= 0 && i < buttons.length && j >= 0 && j < buttons.length) {
                                if (!buttons[i][j].getText().equals("\uD83C\uDF3C")
                                        || buttons[i][j].getText().equals(lion.getEmoji())) { // ignore grass & same type

                                    int finalI = i;
                                    int finalJ = j;
                                    buildingss.forEach(building -> {
                                        if (building.getX() == finalI && building.getY() == finalJ) {
                                            ProduceType type = building.getProduceTypes().get(0);
                                            lion.getInventory().put(type.getName(), lion.getInventory().getOrDefault(type.getName(), 0) + type.getCount());
