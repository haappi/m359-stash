  ...
  public void initialize() {
    ...
    SwitchButton switchButton =
        new SwitchButton(
            "Light Mode",
            "Dark Mode",
            Boolean.parseBoolean((String) Config.getInstance().getConfig().get("darkModeEnabled")));
    switchButton.addListener(
        (old, oldd, newe) -> {
          ...
          try {
            Storage.getInstance()
                .setConfigValue(
                    Config.getInstance().getEmail(),
                    "darkModeEnabled",
                    String.valueOf((!old.getValue().booleanValue())));
          } catch (IOException e) {
    ...
    ObservableList<Swatching> listInstance =
        FXCollections.observableArrayList(Swatching.getAllAsList());
