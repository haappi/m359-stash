...
public class Common {
  ...
  // a hashmap is just a dictionary, everythings assigned to a key, and you can get the value by
  // using the key (think of an actual dictionary)
  /**
  ...
  public static <E> String arrayToString(List<E> array, String separator) {
    ...
    for (E element : array) {
      newString +=
          element
              + separator; // This is a "for each" loop. It iterates through each element in the
                           // array, without having me to specify the index
    }
  ...
  /**
   * Checks if the inputted number is an Armstrong number An Armstrong number is a number that is
   * equal to the sum of number to power of length of its digits <i>This is also known also as a
   * Narcissistic number</i>
   *
  ...
  public static boolean isArmstrongNumber(int number) {
    ...
    for (int i = 0; i < length; i++) {
      addedUp +=
          Math.pow(
              Integer.parseInt(String.valueOf(String.valueOf(number).charAt(i))),
              length); // raise the number to the power of the length of the number
    }
    return addedUp == number;
  }

  /**
   * Checks if the inputted number is considered a Strong number A Strong number is a number that is
   * equal to the sum of factorials of its digits
   *
   * @param number the number to check
  ...
  /**
   * Calculates PI using Leibniz's formula. Rounded to <span color="aqua">2</span> decimal places.
   *
   * @return PI
   */
  @SuppressWarnings("unused")
  ...
  /**
   * Simulates a dice roll for you, and returns the <span color="aqua">max, min, & average</span>.
   * <br>
   *
   * <p>All parameters can be @{@link Nullable}
   *
   ...
   */
  public static String rollADice(
      @Nullable Integer numberOfDice, @Nullable Integer numberOfSides, @Nullable Integer times) {
