...
public class CalculateRootsOfQuadraticEquation {
  ...
  @FXML protected TextField c;
  @FXML protected TextField b;
  @FXML protected TextField a;
  ...
  protected void find() {
    if (!checkIfValuesExist()) {
      ...
      if (discriminant < 0) {
        ...
        output.setText(
            "2 real roots: "
                + format(getRoot1(aValue, bValue, cValue))
                + " and "
                + format(getRoot2(aValue, bValue, cValue)));
      }
    }
  }
