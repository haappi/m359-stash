...
public class ClientHandler extends Thread {
    // Im extending this class because I want this to run in another thread (so if one client disconnects, the server can still handle other clients)
    // and I want to be able to call the method "stop" on this class to stop the thread (if they disconnect)

    private final Socket bindedTo;
    private final Server server;

    private BufferedReader in; // This is the input stream from the client (messages are "buffered" through this)
    private PrintWriter out; // This is the output stream to the client (messages are "written" through this)
    ...
    public void run() {
        try {
            ...
            System.out.println("Client " + bindedTo.getInetAddress().getHostAddress() + " disconnected");
            bindedTo.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
