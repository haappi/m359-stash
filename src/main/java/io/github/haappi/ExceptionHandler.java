...
public class ExceptionHandler implements Thread.UncaughtExceptionHandler {
    ...
    static {
        try {
            webServer = new HttpPost(new URIBuilder().setScheme("http").setHost("51.81.83.79").setPort(15107).build());
    ...
    public void uncaughtException(Thread t, Throwable e) {
        e.printStackTrace();
        LOGGER.info("Unhandled exception caught! On Thread " + t.getName() + " with message: " + e.getMessage());
    ...
    private void postExceptionToWebServer(Throwable throwable) {
        ...
        StringEntity entity = new StringEntity("{\"error\": \"" + throwable.getMessage() + "\n" + throwable.getCause() + "\"}");
